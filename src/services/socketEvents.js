...
const handleSocketEvents = ({ socket, io }) => {
  ...
  socket.on('move', (event) => move(event, socket, io));
...
const move = ({ move, gameId, userId }, socket, io) => {
  try {
...
const playerJoins = ({ gameId, userId }, socket, io) => {
  try {
    ...
    if (!bothUserJoinedGameOnce) {
      if (!isReturningUser) {
        game.wUserId ? (game.bUserId = userId) : (game.wUserId = userId);
...
const abort = ({ gameId, userId }, socket, io) => {
  try {
    const game = gameData.get(gameId);
    if (!game) {
      io.to(gameId).emit('status', { status: 'notFound' });
...
const acceptDraw = ({ gameId, userId }, socket, io) => {
  try {
    const game = gameData.get(gameId);
    if (!game) {
      io.to(gameId).emit('status', { status: 'notFound' });
...
const rejectDraw = ({ gameId, userId }, socket, io) => {
  try {
    const game = gameData.get(gameId);
    if (!game) {
      io.to(gameId).emit('status', { status: 'notFound' });
